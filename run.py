...
class Run:
    ...
    def get_model(self):
        if self.base_model == "MF":
            model = MFBasedModel(
                self.uid_all, self.iid_all, self.num_fields, self.emb_dim, self.meta_dim
            )
    ...
    def get_optimizer(self, model):
        ...
        optimizer_meta = torch.optim.Adam(
            params=model.meta_net.parameters(), lr=self.lr, weight_decay=self.wd
        )
    ...
    def train(
        self, data_loader, model, criterion, optimizer, epoch, stage, mapping=False
    ):
    ...
    def CDR(
        ...
        for i in range(self.epoch):
            self.train(
                data_meta, model, criterion, optimizer_meta, i, stage="train_meta"
            )
